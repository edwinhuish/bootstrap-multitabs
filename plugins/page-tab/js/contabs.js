if(undefined === EDWIN) var EDWIN = {};       //全局变量
...
$(function () {
    ...
    function tabCreate(url,param) {
        ...
        var editContent = $('#J_mainContent .J_content.content-edit');
        if(param.content == 'content-edit' && $(editContent).length && !$(editContent).find('button.btn-save').hasClass('disabled')){
            if(typeof window.swal == 'function'){
                swal("Warning!", "Please colse or save the Editor before open the other!", "warning")
            }else alert("Please colse or save the Editor before open the other!");
            window.location.hash= encodeURI($(editContent).attr('data-id'));
            return false;
        }
        url = url.replace('#', '');
        ...
        var content = $('#J_mainContent .J_content.'+param.content+'[data-index="'+index+'"]');
        ...
        if($(content).length > 0 && $(content).is('iframe') != param.iframe) $(content).remove(); //如果模式不同，则删除再进行判断。
        if($(content).length <= 0){
        ...
        }else if($(content).is('iframe') != param.iframe){
    ...
    function tabClose() {
        ...
        var content = $('#J_mainContent .J_content[data-id="'+id+'"]');
        if($(this).parents(".J_menuTab").hasClass('content-edit') && !$(content).find('button.btn-save').hasClass('disabled')){
    ...
    function tabEnable() {
        if (!$(this).hasClass("active")) {
            window.location.hash= encodeURI($(this).attr("data-id"));
        }
    }
